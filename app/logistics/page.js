...
export default function LogisticsDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-300 p-4 lg:p-8 font-sans selection:bg-emerald-500/30 selection:text-emerald-200">
      <div className="max-w-7xl mx-auto space-y-8 animate-fade-in-up">
        
        {/* Header */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-slate-900/50 border border-slate-800 p-8 rounded-[3rem] relative overflow-hidden group">
          ...
          <div className="flex items-center gap-6 relative z-10">
            <div className="w-16 h-16 rounded-3xl bg-emerald-500 shadow-[0_0_50px_rgba(16,185,129,0.3)] text-white flex items-center justify-center group-hover:scale-105 transition-transform">
            ...
            <div>
              ...
              <h1 className="text-4xl font-black text-white tracking-tighter">Logistics Hub</h1>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 relative z-10 w-full md:w-auto">
             <div className="bg-slate-950 px-6 py-4 rounded-3xl border border-slate-800 text-center flex-1 md:flex-none">
                ...
                <p className="text-2xl font-black text-white leading-none">{pendingPickups.length}</p>
             </div>
             <div className="bg-slate-950 px-6 py-4 rounded-3xl border border-slate-800 text-center flex-1 md:flex-none">
                <p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest mb-1">Deliveries</p>
                <p className="text-2xl font-black text-white leading-none">{pendingDeliveries.length}</p>
             </div>
          </div>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {pendingPickups.length === 0 ? (
                <div className="p-12 text-center bg-slate-900/30 rounded-[3rem] border border-dashed border-slate-800">
                ...
                <div key={'p-'+task.id} className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-6 hover:border-slate-700 transition-colors shadow-sm animate-scale-in" style={{ animationDelay: (i * 50) + 'ms' }}>
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      ...
                      <h3 className="text-xl font-black text-white mt-3 leading-tight">{task.customer_name}</h3>
                  ...
                  <div className="bg-slate-950 rounded-3xl p-5 mb-6 border border-slate-800/50 flex gap-4">
                    <span className="material-symbols-outlined text-slate-600 mt-0.5">location_on</span>
                    <p className="text-sm font-bold text-slate-300 leading-relaxed">{task.customer_address}</p>
                  </div>
                  ...
                  <div className="flex gap-3">
                    {task.pickup_status === 'pending' || task.pickup_status === 'scheduled' ? (
                      <button 
                        ...
                        className="flex-1 bg-amber-600 hover:bg-amber-500 text-white rounded-2xl py-4 text-[10px] font-black uppercase tracking-widest transition-all"
                      >
                        Start Route
                      </button>
                    ) : (
                      <button 
                        ...
                        className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl py-4 text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(16,185,129,0.2)]"
                      >
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {pendingDeliveries.length === 0 ? (
                ...
                <div key={'d-'+task.id} className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-6 hover:border-slate-700 transition-colors shadow-sm animate-scale-in" style={{ animationDelay: (i * 50) + 'ms' }}>
                  <div className="flex justify-between items-start mb-6">
                    <div>
                  ...
                  <div className="flex gap-3">
                    {task.delivery_status === 'pending' || task.delivery_status === 'scheduled' ? (
                      <button 
                        ...
                        className="flex-1 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl py-4 text-[10px] font-black uppercase tracking-widest transition-all"
                      >
                        Start Route
                      </button>
                    ) : (
