...
export async function GET() {
  try {
    ...
    // Execute all staff analytics queries in parallel to minimize cross-region latency
    const [summaryRes, staffStatsRes, dailyThroughputRes, bottlenecksRes] = await Promise.all([
      // 1. Overall Summary
      query(`
        WITH cycle_times AS (
          SELECT 
    ...
    // Cache for 1 minute as staff data changes more frequently but still benefits from edge caching
