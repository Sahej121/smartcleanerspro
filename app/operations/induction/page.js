...
export default function InductionPage() {
  ...
  const saveInduction = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(err.error || 'Failed to save induction');
      }
    } catch (err) {
  ...
  if (authLoading || loading) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
      ...
      <p className="text-slate-400 font-bold uppercase tracking-widest animate-pulse">Loading Induction Queue...</p>
    </div>
  );
    ...
    <div className="p-4 lg:p-8 space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex justify-between items-end">
        ...
        <div className="bg-emerald-50 text-emerald-700 px-6 py-3 rounded-2xl border border-emerald-100 flex items-center gap-3">
          ...
          <span className="text-sm font-black uppercase tracking-widest">{orders.length} Orders Pending</span>
        </div>
      </div>
          ...
          >
            ...
            <div className="flex justify-between items-start mb-6 relative z-10">
               <div>
                 ...
                 <h3 className="text-xl font-black text-on-surface mt-2 leading-tight">{order.customer_name || 'Walk-in'}</h3>
          ...
          <div className="bg-surface rounded-[3rem] w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl border border-white/20">
            ...
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
               <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h2 className="text-2xl font-black text-on-surface">Inducting Order {selectedOrder.order_number}</h2>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8 no-scrollbar bg-slate-50/30">
               ...
               <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col md:flex-row gap-8 items-center">
                  <div className="flex-1">
                    <h3 className="text-lg font-black text-on-surface mb-1">Assign Bag ID</h3>
                    <p className="text-xs font-medium text-slate-400 uppercase tracking-widest">All garments in this order will be placed in this bag.</p>
                  </div>
                  <div className="w-full md:w-64 relative">
                    ...
                    <input 
                      ...
                      placeholder="Scan Bag QR (e.g. B12)"
               ...
               <div className="grid grid-cols-1 gap-4">
                  <h3 className="text-sm font-black text-slate-500 uppercase tracking-[0.3em] pl-4 mb-2">Garment Tagging & Pricing</h3>
                  {inductionData.items.map((item, i) => (
                    <div key={item.id} className="bg-white rounded-[2.5rem] border border-slate-100 p-6 flex flex-col md:flex-row gap-8 items-center group hover:border-emerald-200 transition-colors">
                       ...
                       <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
                          <div>
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Garment & Service</label>
                          ...
                          <div className="space-y-2">
                             <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Unit Price (₹)</label>
                          ...
                          <div className="space-y-2">
                             <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Plastic Card QR (Tag ID)</label>
                             <div className="relative">
            ...
            <div className="p-8 border-t border-slate-100 bg-white flex justify-between items-center">
               <div className="hidden md:block">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Estimated Amount</p>
