...
export default function PricingContent() {
  ...
  const handleStart = (tierKey) => {
    ...
    const priceStr = getTierPrice(tierKey, selectedMarket, billingCycle);
    // Extract numerical value, handling commas and currency symbols
    const basePrice = parseFloat(priceStr.replace(/[^0-9.]/g, ''));
    
    router.push(`/checkout?tier=${tierKey}&market=${selectedMarket}&planName=${encodeURIComponent(tier.label)}&price=${basePrice}&billingCycle=${billingCycle}`);
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8FAF9] text-slate-900 font-sans selection:bg-emerald-500/30 overflow-x-hidden">
      ...
      <header className="relative z-10 pt-20 pb-16 text-center px-6">
        ...
        {/* Pricing Grid Header & Toggle */}
        <div className="relative z-20 max-w-4xl mx-auto mb-12 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-300 flex flex-col items-center gap-10">
          {isDetecting ? (
          ...
          <div className="flex items-center gap-4 p-1.5 bg-slate-900/5 rounded-full border border-slate-200/50 backdrop-blur-md">
            <button 
              onClick={() => {
              ...
              className={`px-8 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${billingCycle === 'monthly' ? 'bg-white text-slate-900 shadow-xl shadow-slate-200' : 'text-slate-400 hover:text-slate-600'}`}
            >
              {t('monthly')}
            </button>
            <button 
              ...
              className={`relative px-8 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${billingCycle === 'yearly' ? 'bg-white text-slate-900 shadow-xl shadow-slate-200' : 'text-slate-400 hover:text-slate-600'}`}
            >
              {t('yearly')}
              <span className="absolute -top-3 -right-3 px-2 py-1 bg-emerald-500 text-white text-[8px] font-black rounded-lg shadow-lg animate-bounce">
                -15% OFF
              </span>
            </button>
          </div>
              ...
              <div className="mb-10">
                ...
                <div className={`flex items-baseline gap-1 transition-all duration-500 ${isPriceAnimating ? 'opacity-0 scale-95 translate-y-2' : 'opacity-100 scale-100 translate-y-0'}`}>
                  <span className="text-5xl font-black text-slate-900 tracking-tighter">
                    {getTierPrice(key, selectedMarket, billingCycle)}
                  </span>
                  {key !== 'enterprise' ? (
