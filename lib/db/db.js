...
async function initializeDb() {
  ...
  try {
    ...
    `);
    await p.query(`ALTER TABLE order_items ADD COLUMN IF NOT EXISTS image_url TEXT;`);
    await p.query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS bag_id TEXT;`);
    await p.query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS driver_id INTEGER REFERENCES users(id);`);
    await p.query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS signature_data TEXT;`);
    await p.query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS proof_photo_url TEXT;`);
    await p.query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS logistics_last_updated TIMESTAMPTZ DEFAULT NOW();`);
  } catch (e) {
    // Ignore duplicate column errors, but log others
    if (!e.message.includes('already exists')) {
      console.error('[DB] Logistics migration warning:', e.message);
    }
  }
}
...
async function seedDatabase(p) {
  ...
  await p.query(
    ...
    ['DrycleanersFlow Main', '123 MG Road', 'New Delhi', 'India', '+91-9876543210']
  );
