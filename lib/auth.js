  ...
  try {
    ...
    if (user && !error) {
      ...
      const res = await query(
        'SELECT u.*, s.subscription_tier, s.status as store_status FROM users u LEFT JOIN stores s ON u.store_id = s.id WHERE u.auth_id = $1',
        [user.id]
      );
    ...
    if (pinToken) {
      ...
      if (payload) {
        // For PIN sessions, we still want to check suspension status if possible
        // but to keep it "fast" we can trust the JWT for other fields.
        let isSuspended = false;
        if (payload.store_id) {
          const storeRes = await query('SELECT status FROM stores WHERE id = $1', [payload.store_id]);
