...
/**
 * Validates a PIN-based session token.
 */
...
export const getSession = cache(async () => {
  try {
    ...
    // Fast check: getSession() first as it's often satisfies by local cookies/state
...
export async function requireRole(request, allowedRoles) {
  ...
  // App-level superuser check (Owner with ID 1)
  const isSaasOwner = session.role === 'owner' && session.id === 1;
