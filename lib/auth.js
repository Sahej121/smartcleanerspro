...
if (!process.env.SUPABASE_AUTH_JWT_SECRET && !process.env.JWT_SECRET) {
...
export const getSession = cache(async () => {
  try {
    ...
    const res = await query(
      `SELECT u.*, s.subscription_tier, s.status as store_status, s.store_name, s.branding 
       FROM users u 
    ...
    if (res.rows.length > 0) {
      ...
      return {
        ...
        role: appUser.role, // Reliably use the DB-assigned role
...
export async function requireRole(request, allowedRoles) {
  ...
  // App-level superuser check — only the dedicated 'superadmin' role bypasses restrictions
  const isSaasOwner = session.role === 'superadmin';

  const userRole = session.role?.toLowerCase();
  if (!isSaasOwner && !allowedRoles.includes(userRole)) {
