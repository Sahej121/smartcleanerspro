...
export function UserProvider({ children, initialUser = null }) {
  ...
  // Background polling to check for suspension/updates every 5 minutes when online
  useEffect(() => {
    if (!user) return;
    ...
    const handleFocus = () => {
      if (window.navigator.onLine) fetchUser();
    };
  ...
  const logout = async () => {
    // Clear server-side session FIRST, then clear client state
    try {
    ...
    // Clear client state after cookies are destroyed
    setUser(null);
    setRole(null);
    router.push('/login');
  };
