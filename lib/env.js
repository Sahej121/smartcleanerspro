...
if (!_env.success) {
  if (isServer) {
    const isBuild = process.env.NODE_ENV === 'production' && (process.env.VERCEL === '1' || process.env.CI === 'true');
    
    console.error('❌ Invalid environment variables:', JSON.stringify(_env.error.format(), null, 2));
    
    if (isBuild) {
      console.warn('⚠️ Build-time validation failed. This is acceptable for sensitive keys not needed during build, but they MUST be provided at runtime.');
    } else {
