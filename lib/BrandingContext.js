...
export function BrandingProvider({ children }) {
  const { user } = useUser();
  const [systemName, setSystemName] = useState('DrycleanersFlow');
  ...
  useEffect(() => {
    ...
    if (user && normalizeTier(user.tier) === 'software_only') {
      setSystemName("Drycleaner's Flow");
      localStorage.setItem('cleanflow_system_name', "Drycleaner's Flow");
    } else {
      if (savedName) setSystemName(savedName);
      else {
        setSystemName('DrycleanersFlow');
        localStorage.setItem('cleanflow_system_name', 'DrycleanersFlow');
      }
    }
