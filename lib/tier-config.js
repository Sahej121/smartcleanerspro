...
export function getMarketForCountry(country = '') {
 ...
 */
export function getTierPrice(tierKey, marketId = 'us', billingCycle = 'monthly') {
  if (tierKey === 'enterprise') return 'Contact Sales';
  const market = PRICING_MARKETS[marketId] || PRICING_MARKETS.us;
  const priceStr = market.prices[tierKey];
  const basePrice = parseFloat(priceStr.replace(/,/g, ''));
  
  if (billingCycle === 'yearly') {
    // 15% discount for yearly: (Monthly * 12) * 0.85
    const yearlyPrice = (basePrice * 12 * 0.85);
    // Format with commas if needed
    const formattedYearly = Math.round(yearlyPrice).toLocaleString();
    return `${market.currency}${formattedYearly}`;
  }
  
  return `${market.currency}${priceStr}`;
}
